...
app.route('/autocomplete/:method/').get((req, res) => {
    ...
    if (dbScheme[table]) {
        ...
        switch (table) {
            case 'catalogo':
                sql = `SELECT 'idcatalogo' as field, idcatalogo as value, nombre as label, codigo  from ${table} WHERE lower(nombre) LIKE lower('${q}%') OR lower(marca) LIKE lower('${q}%') OR lower(descripcion) LIKE lower('${q}%')`;
                break;
            case 'medidas':
                sql = `SELECT 'idmedida' as field,  idmedida as value, nombre as label from ${table} WHERE lower(abreviatura) LIKE lower('${q}%') OR lower(nombre) LIKE lower('${q}%') OR lower(descripcion) LIKE lower('${q}%')`;
                break;
            case 'objeto_gasto':
